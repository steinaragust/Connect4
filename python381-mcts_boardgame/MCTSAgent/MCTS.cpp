...
inline double UCT(TreeNodeLabel* parent, TreeNodeLabel* child) {
  if (child->get_n() == 0) return numeric_limits<double>::max();
...
inline double calculate_child_value(TreeNodeLabel* parent, TreeNodeLabel* child, int index, bool use_PUCT) {
  return use_PUCT ? PUCT(parent, child, index) : UCT(parent, child);
}
...
inline void playout (BoardGame &game, HashMapTree* tree, vector<TreeNodeLabel*> &path) {
  while (!game.is_terminal_state()) {
    ...
    TreeNodeLabel* node = tree->add_node_if_not_exists(key);
    if (node == NULL) node = tree->get_node_label(key);
    path.push_back(node);
    node->add_visit();
  }
}
