...
class MCTS_Evaluator:
  def __init__(self, predictPtr, agent_1_model_nr, agent_2_model_nr, simulations = 200):
    ...
    self.model_1 = ResNet()
    self.load_model_for_agent(self.model_1, agent_1_model_nr)
    self.model_2 = ResNet()
    self.load_model_for_agent(self.model_2, agent_2_model_nr)
    self.model_1.eval()
    self.model_2.eval()
    self.reset()
  ...
  def load_model_for_agent(self, agent_model, model_n):
    ...
    agent_model.load_state_dict(torch.load(fpath))

  def predict(self, states, values, n_states):
    tensor_arr = tensor(states, dtype=torch.float)
    model = self.model_1 if self.turn == 0 else self.model_2
...
evaluator = MCTS_Evaluator(predict, 0, 0)

score = evaluator.play_matches(2)
